...
public class PassbookUi {
	...
	private static void accountSummary() {
		...
		if (acc != null) {
			...
			acc.getTransactions().stream()
					.filter(t -> t.getTransactionDate().isAfter(LocalDate.of(y1, m1, d1))
							&& t.getTransactionDate().isBefore(LocalDate.of(y2, m2, d2)))
	...
	private static void getPassbookInfo() {
		...
		if (acc != null) {
			System.out.println("Account id : " + acc.getAccountId());
			System.out.println("Current balance : " + acc.getBalance());
			acc.getTransactions().stream().filter(t -> t.getTransactionDate().isBefore(LocalDate.now()))
					.forEach(t -> System.out.println(t));
